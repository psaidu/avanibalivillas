...
// ── CONFIG ──────────────────────────────────────────────────
var SEASONAL_PRICES = [
  { monthStart:1,  monthEnd:3,  price:228 },
  { monthStart:4,  monthEnd:6,  price:260 },
  { monthStart:7,  monthEnd:9,  price:292 },
  { monthStart:10, monthEnd:12, price:249 },
];
...
async function adminDeleteBooking(ds) {
  ...
  if (!confirmed) return;
  try {
    var r = await fetch('/.netlify/functions/bellis-bookings');
    ...
    var res = await fetch('/.netlify/functions/delete-booking', {
      method:'POST', headers:{'Content-Type':'application/json'},
      body: JSON.stringify({ villa:'bellis', password:ADMIN_PASSWORD, bookingId:bookingId })
    });
...
async function reloadCalendar() {
  ...
  try {
    var results = await Promise.all([
      fetch('/.netlify/functions/ical?villa=bellis&nocache='+Date.now()).then(function(r){return r.ok?r.text():'';}).catch(function(){return'';}),
      fetch('/.netlify/functions/bellis-bookings?nocache='+Date.now()).then(function(r){return r.ok?r.text():'';}).catch(function(){return'';})
    ]);
...
async function initCalendar() {
  ...
  try {
    var results = await Promise.all([
      fetch('/.netlify/functions/ical?villa=bellis').then(function(r){return r.ok?r.text():'';}).catch(function(){return'';}),
      fetch('/.netlify/functions/bellis-bookings').then(function(r){return r.ok?r.text():'';}).catch(function(){return'';}),
...
function switchBank(bank) {
  ['bca','wise','zelle'].forEach(function(b) {
    var panel = document.getElementById('bank-'+b);
    var btn   = document.getElementById('bank-'+b+'-btn');
    if (panel) panel.style.display = bank===b ? 'block' : 'none';
    if (btn)   btn.classList.toggle('active', bank===b);
  });
  updateBankReference();
}
...
function updateBankReference() {
  ...
  ['bca-ref','wise-ref','zelle-ref'].forEach(function(id) {
